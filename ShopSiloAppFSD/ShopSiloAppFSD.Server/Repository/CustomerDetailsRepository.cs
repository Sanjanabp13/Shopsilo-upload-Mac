...
namespace ShopSiloAppFSD.Repository
{
    public class CustomerDetailsRepository : ICustomerDetailsRepository
    {
        ...
        public async Task ToggleCustomerStatusAsync(int customerId)
        {
            try
            {
                ...
                // Optional: Add audit log if needed
                if (_auditLogConfig.IsAuditLogEnabled) // Check if audit log is enabled
                {
                    var auditLog = new AuditLog
                    {
                        Action = $"Customer status toggled for CustomerID {customerId}. New Status: {(customer.IsActive ? "Active" : "Inactive")}",
                        Timestamp = DateTime.Now,
            ...
            {
                throw new RepositoryException("Error toggling customer status.", ex);
            }
            catch (Exception ex)
            {
                throw new RepositoryException("Error toggling customer status.", ex);
