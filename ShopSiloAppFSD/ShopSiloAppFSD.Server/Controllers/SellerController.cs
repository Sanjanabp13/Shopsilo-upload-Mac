...
namespace ShopSiloAppFSD.Controllers
{
    ...
    public class SellerController : ControllerBase
    {
        ...
        public async Task<IActionResult> ToggleSellerStatus(int sellerId)
        {
            try
            {
                await _sellerRepository.ToggleSellerStatusAsync(sellerId);
                return NoContent(); // No content to return on success
            }
            catch (NotFoundException)
            {
                return NotFound(); // Return 404 if the seller is not found
            }
            catch (Exception ex)
            {
                return StatusCode(500, $"Internal server error: {ex.Message}"); // Handle other errors
            }
        }
